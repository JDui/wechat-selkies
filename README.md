...
- `/config` 持久化、中文字体、本地输入、中文 IME 组合上屏、剪贴板、图片粘贴
  和大文件上传。
...
输入框编辑或聚焦时菜单不会自动收纳，也不会被后台连接状态覆盖。输入控件保持为
页面可识别的本地表单元素，远端 Selkies 不会抢走其 IME 焦点或截获菜单内的
`Ctrl+C/X/V`。文字、图标和输入区域使用高对比度深色玻璃背景，保证叠加在深浅
网页上都可辨认。

客户端和容器页面都保护中文输入法的组合事件：拼音候选期间不发送临时文本，
选中候选词后只向远端提交最终文字一次。

文件上传器会在共享登录状态的独立 WebView2 窗口中打开，网页拖放上传可直接
接收本机文件。下载会保存到 Windows 当前配置的“下载”目录；普通网页弹窗和
`target="_blank"` 链接则交给系统默认浏览器，不会留在客户端内形成空白窗口。
