...
- **通知中心**：右侧可收纳通知中心集中显示微信、QQ、剪板、系统、工具和链接事件；推流流量总计保留在通知中心顶部摘要，实时网速与往返延迟移至快捷 Bar 左端，不再刷屏进入历史列表，同类系统级通知会在 1 分钟内合并为最新一条。
...
- 「妙妙小工具」中提供「网速与往返延迟」开关，独立于通知中心开关，设置持久保存。零流量会实时归零，断线或超过 10 秒未收到样本时显示待更新占位。
- 修正网速显示中 Kbit/s 到 KB/s 的换算。
- 快捷 Bar、收纳按钮与分屏菜单统一为液态玻璃样式，提供高光描边、半透明色彩和按压反馈；适配顶 / 底位置、窄屏和减少动态效果偏好。收纳后仅展开按钮接收点击。

维护说明：上游前端以预编译 bundle 形式纳入仓库，本次直接修改其中原生帧率控件，沿用原有设置回调和服务端限制；更新上游 bundle 时需保留此改动。相关回归检查：`node tests/test_dock_network_monitor.js`、`node tests/test_preferences_client.js`、`node tests/test_sidebar_scroll_render.js`、`python -m unittest discover -s tests -p test_sleep_preferences.py`。
