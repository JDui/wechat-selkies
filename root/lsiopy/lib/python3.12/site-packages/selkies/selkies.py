...
class DataStreamingServer:
    ...
    def _refresh_low_latency_runtime_tuning(self, display_state):
        ...
        low_latency_send_fps = max(8.0, min(base_fps, low_latency_fps))
        ...
        if disable_paint_over:
            high_load_send_fps = min(high_load_send_fps, low_latency_send_fps - 1.0)

        input_priority_seconds = 0.10 + (transport_level * 0.18) + (sample_penalty * 0.08)
        if disable_paint_over:
            input_priority_seconds += 0.04

        display_state["low_latency_send_fps"] = max(8.0, min(base_fps, low_latency_send_fps))
        display_state["low_latency_bandwidth_percent"] = bandwidth_percent
        display_state["high_load_send_fps"] = max(6.0, min(display_state["low_latency_send_fps"], high_load_send_fps))
                ...
                if isinstance(message, bytes):
                    ...
                    if message.startswith("FILE_UPLOAD_START:"):
                        ...
                        try:
                            ...
                            if display_id not in self.display_clients:
                                ...
                                self.display_clients[display_id] = {
                                    ...
                                    'low_latency_send_fps': min(15, max(8, int(self.app.framerate))),
                                    'low_latency_bandwidth_percent': max(75, max(10, min(100, int(round((min(15, max(8, int(self.app.framerate))) / max(1, int(self.app.framerate))) * 100.0))))),
                        ...
                        if client_display_id and client_display_id in self.display_clients:
                            ...
                            self._refresh_low_latency_runtime_tuning(display_state)
                            old_active = bool(display_state.get("dynamic_low_latency_active"))
                            ...
                            if old_active != bool(display_state["dynamic_low_latency_active"]):
