...
class DataStreamingServer:
    ...
    def _refresh_low_latency_runtime_tuning(self, display_state):
        ...
        low_latency_fps = float(display_state.get("dynamic_low_latency_fps", base_fps) or base_fps)
        try:
        ...
        low_latency_send_fps = min(base_fps, low_latency_fps)
        low_latency_send_fps = max(8.0, low_latency_send_fps - (transport_level * 8.0))
    ...
    async def _apply_client_settings(
        ...
        def sanitize_low_latency_fps(value):
            if value is None:
                return old_settings.get("dynamic_low_latency_fps", max(12, int(self.app.framerate)))
            try:
                numeric = int(value)
            except (TypeError, ValueError):
                return old_settings.get("dynamic_low_latency_fps", max(12, int(self.app.framerate)))
        ...
        async with self._reconfigure_lock:
            ...
            display_state["dynamic_low_latency_sample_percent"] = sanitize_sample_percent(settings.get("dynamic_low_latency_sample_percent"))
                    ...
                    if message.startswith("FILE_UPLOAD_START:"):
                        ...
                        if client_display_id and client_display_id in self.display_clients:
                            ...
                            if display_state["dynamic_low_latency_active"]:
                                self._register_input_priority(
                                    client_display_id,
                                    display_state.get("input_priority_seconds", 0.24),
                                )
                                self._prune_video_queue_for_display(client_display_id, LOW_LATENCY_QUEUE_KEEP_LATEST)
                            else:
                                display_state["input_priority_until"] = 0.0
                            data_logger.info(
                                f"Dynamic low-latency {'enabled' if display_state['dynamic_low_latency_active'] else 'disabled'} for '{client_display_id}'."
                            )
    ...
    def _get_capture_settings(self, display_id, width, height, x, y):
        ...
        try:
            dynamic_sample_percent = max(33, min(100, int(dynamic_sample_percent)))
        except (TypeError, ValueError):
            dynamic_sample_percent = 100
