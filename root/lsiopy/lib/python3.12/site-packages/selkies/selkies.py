...
class DataStreamingServer:
    ...
    def _refresh_low_latency_runtime_tuning(self, display_state):
        ...
        disable_paint_over = bool(display_state.get("dynamic_low_latency_disable_paint_over", True))

        low_latency_send_fps = max(1.0, min(base_fps, low_latency_fps))
        bandwidth_percent = max(10, min(100, int(round((low_latency_send_fps / max(1.0, base_fps)) * 100.0))))
        bandwidth_percent = max(bandwidth_percent, sample_percent)
                    ...
                    if message.startswith("FILE_UPLOAD_START:"):
                        ...
                        if client_display_id and client_display_id in self.display_clients:
                            ...
                            if not new_low_latency_active:
                                display_state["input_priority_until"] = 0.0
                            data_logger.info(
                                f"Dynamic inactive frame limit {'enabled' if new_low_latency_active else 'disabled'} for '{client_display_id}'."
                            )
                            if (
