...
TARGET_FRAMERATE = 30
LOW_LATENCY_INPUT_PRIORITY_SECONDS = 0.18
LOW_LATENCY_QUEUE_KEEP_LATEST = 1
DEFAULT_STREAM_LOAD_SEND_FPS = 18.0
...
GENERIC_AUDIO_SOURCE_NAMES = {"", "auto", "default", "output.monitor"}
PREFERRED_AUDIO_SOURCE_NAMES = ("selkies_output.monitor", "output.monitor")
GPU_ID_DEFAULT = 0
...
class DataStreamingServer:
    ...
    def __init__(
        ...
        self._bytes_received_in_interval = 0
        ...
        self.low_latency_input_priority_seconds = LOW_LATENCY_INPUT_PRIORITY_SECONDS

        self.display_clients = {}
        self.video_chunk_queues = {}
        self.capture_instances = {}
        self.awake_client_states = {}
        self.awake_state_path = os.environ.get(
            "SELKIES_AWAKE_STATE_PATH", "/tmp/selkies-client-awake.json"
        )
        self.adaptive_sleep_state_path = os.environ.get(
            "NOTIFICATION_BRIDGE_MODE_PATH", "/config/state/notification-bridge.json"
        )
        try:
            self.adaptive_sleep_idle_seconds = max(
                5, min(3600, int(os.environ.get("SELKIES_ADAPTIVE_SLEEP_IDLE_SECONDS", "3600") or "3600"))
            )
        except Exception:
            self.adaptive_sleep_idle_seconds = 3600
        try:
            self.adaptive_sleep_check_seconds = max(
                1, min(60, int(os.environ.get("SELKIES_ADAPTIVE_SLEEP_CHECK_SECONDS", "5") or "5"))
            )
        except Exception:
            self.adaptive_sleep_check_seconds = 5
        self._adaptive_sleep_task = None
        self._adaptive_sleeping = False
        self._adaptive_sleep_no_awake_since = 0.0
        self._adaptive_sleep_video_active = {}
        self._adaptive_sleep_audio_active = False
        ...
        self._persist_awake_clients_state()

    def _persist_awake_clients_state(self):
        now = time.time()
        active_items = []
        for state in self.awake_client_states.values():
            updated_at = float(state.get("updated_at", 0.0) or 0.0)
            if updated_at <= 0:
                continue
            active_items.append(
                {
                    "awake": bool(state.get("awake", False)),
        ...
        awake_count = sum(
            ...
            if item["awake"] and (now - item["updated_at"]) <= 45.0
        )
        payload = {
            "updated_at": now,
            "awake_clients": awake_count,
            "any_awake": awake_count > 0,
            "clients": active_items,
        }
        try:
            state_path = pathlib.Path(self.awake_state_path)
            state_path.parent.mkdir(parents=True, exist_ok=True)
            temp_path = state_path.with_suffix(".tmp")
            temp_path.write_text(json.dumps(payload), encoding="utf-8")
            temp_path.replace(state_path)
        except Exception as exc:
            data_logger.warning(f"Failed to persist awake client state: {exc}")

    def _set_client_awake_state(self, websocket, awake: bool, display_id: str | None = None):
        self.awake_client_states[id(websocket)] = {
            "awake": bool(awake),
    ...
    def _adaptive_sleep_enabled(self):
        try:
            payload = json.loads(pathlib.Path(self.adaptive_sleep_state_path).read_text(encoding="utf-8"))
        except Exception:
            return False
        try:
            idle_seconds = int(payload.get("adaptive_sleep_idle_seconds", self.adaptive_sleep_idle_seconds) or self.adaptive_sleep_idle_seconds)
            if idle_seconds in (60, 900, 1800, 2700, 3600):
                self.adaptive_sleep_idle_seconds = idle_seconds
        except Exception:
            pass
        value = payload.get("adaptive_sleep_enabled", False)
        if isinstance(value, bool):
            return value
        return str(value).strip().lower() in ("1", "true", "yes", "on")

    def _receiving_client_count(self):
        count = 0
        for display_state in self.display_clients.values():
            websocket = display_state.get("ws")
            if websocket not in self.clients:
                continue
            if not display_state.get("video_active", False):
                continue
            count += 1
        return count

    def _awake_client_count(self):
        now = time.time()
        count = 0
        for state in self.awake_client_states.values():
            try:
                ...
                updated_at = 0.0
            if state.get("awake") and updated_at > 0 and (now - updated_at) <= 45.0:
                count += 1
        return count

    def _has_sleepable_pipeline_state(self):
        return bool(self.display_clients or self.capture_instances or self.is_pcmflux_capturing)

    async def _enter_adaptive_sleep(self):
        if self._adaptive_sleeping:
            return
        async with self._reconfigure_lock:
            if self._adaptive_sleeping or not self._has_sleepable_pipeline_state():
                return
            data_logger.info("Adaptive sleep: no awake clients, stopping video/audio pipelines.")
            self._adaptive_sleep_video_active = {
                display_id: bool(display_state.get("video_active", False))
                for display_id, display_state in self.display_clients.items()
            }
            self._adaptive_sleep_audio_active = bool(self.is_pcmflux_capturing)
            for display_state in self.display_clients.values():
                display_state["video_active"] = False
            ...
            self._adaptive_sleeping = True
        if self.clients:
            websockets.broadcast(self.clients, "VIDEO_STOPPED")
            websockets.broadcast(self.clients, "AUDIO_STOPPED")

    async def _leave_adaptive_sleep(self, reason="client awake"):
        if not self._adaptive_sleeping:
            return
        data_logger.info(f"Adaptive sleep: waking pipelines ({reason}).")
        for display_id, was_active in self._adaptive_sleep_video_active.items():
            if display_id in self.display_clients:
                self.display_clients[display_id]["video_active"] = bool(was_active)
        self._adaptive_sleeping = False
        self._adaptive_sleep_no_awake_since = 0.0
        await self.reconfigure_displays()
        if self._adaptive_sleep_audio_active and PCMFLUX_AVAILABLE:
            await self._start_pcmflux_pipeline()
        if self.clients:
            if any(self._adaptive_sleep_video_active.values()):
                websockets.broadcast(self.clients, "VIDEO_STARTED")
            if self._adaptive_sleep_audio_active:
                websockets.broadcast(self.clients, "AUDIO_STARTED")

    async def _adaptive_sleep_monitor(self):
        data_logger.info("Adaptive sleep monitor started.")
        while True:
            try:
                await asyncio.sleep(max(1, self.adaptive_sleep_check_seconds))
                enabled = self._adaptive_sleep_enabled()
                awake_count = self._awake_client_count()
                if (not enabled or awake_count > 0) and self._adaptive_sleeping:
                    await self._leave_adaptive_sleep("disabled" if not enabled else "client awake")
                    continue
                if not enabled:
                    self._adaptive_sleep_no_awake_since = 0.0
                    continue
                if not self._has_sleepable_pipeline_state():
                    self._adaptive_sleep_no_awake_since = 0.0
                    self._adaptive_sleeping = False
                    continue
                if awake_count > 0:
                    ...
                    continue
                now = time.time()
                if self._adaptive_sleep_no_awake_since <= 0:
                    self._adaptive_sleep_no_awake_since = now
                    continue
                if (now - self._adaptive_sleep_no_awake_since) >= max(5, self.adaptive_sleep_idle_seconds):
                    await self._enter_adaptive_sleep()
            except asyncio.CancelledError:
                data_logger.info("Adaptive sleep monitor cancelled.")
                raise
            except Exception as exc:
                data_logger.warning(f"Adaptive sleep monitor error: {exc}")

    def _is_low_latency_mode_active(self, display_state):
        return bool(
            display_state
            and (
                display_state.get("dynamic_low_latency_active")
                or display_state.get("high_load_active")
            )
        )

    def _is_h264_display_state(self, display_state):
        if not display_state:
            return False
        return display_state.get("encoder", self.app.encoder) in ("x264enc", "x264enc-striped")

    def _should_preserve_encoded_frame_order(self, display_state):
        return self._is_h264_display_state(display_state)

    def _coerce_int_setting(self, value, fallback, min_value, max_value):
        try:
            parsed = int(value)
    ...
    def _register_input_priority(self, display_id: str, duration_s: float | None = None):
        ...
        hold_for = (
            ...
            else display_state.get("input_priority_seconds", self.low_latency_input_priority_seconds)
        )
        try:
            hold_for = max(0.05, float(hold_for))
        except (TypeError, ValueError):
            hold_for = self.low_latency_input_priority_seconds
        display_state["input_priority_until"] = time.monotonic() + hold_for

    def _set_display_high_load_state(self, display_id: str, active: bool, reason: str = ""):
        display_state = self.display_clients.get(display_id)
        if not display_state:
            return
        new_value = bool(active)
        old_value = bool(display_state.get("high_load_active"))
        if old_value == new_value:
            return
        display_state["high_load_active"] = new_value
        if new_value:
            self._register_input_priority(display_id, 0.45)
            if not self._should_preserve_encoded_frame_order(display_state):
                self._prune_video_queue_for_display(display_id, LOW_LATENCY_QUEUE_KEEP_LATEST)
        if old_value != new_value:
            state_text = "enabled" if new_value else "cleared"
            suffix = f" ({reason})" if reason else ""
            data_logger.info(f"High-load transport mode {state_text} for '{display_id}'{suffix}.")

    def _prune_video_queue_for_display(self, display_id: str, keep_latest: int | None = None):
        queue = self.video_chunk_queues.get(display_id)
        if not queue:
            return
        display_state = self.display_clients.get(display_id, {})
        if self._should_preserve_encoded_frame_order(display_state):
            return
        if keep_latest is None:
            keep_latest = LOW_LATENCY_QUEUE_KEEP_LATEST if self._is_low_latency_mode_active(display_state) else 4
        keep_latest = max(1, int(keep_latest))
        while queue.qsize() >= keep_latest:
            try:
                queue.get_nowait()
    ...
    def _get_effective_send_interval_seconds(self, display_state):
        if not display_state:
            return 0.0
        if self._should_preserve_encoded_frame_order(display_state):
            return 0.0
        target_fps = float(display_state.get("framerate", self.app.framerate) or self.app.framerate or TARGET_FRAMERATE)
        if display_state.get("high_load_active"):
            target_fps = min(
                target_fps,
                float(display_state.get("high_load_send_fps", DEFAULT_STREAM_LOAD_SEND_FPS) or DEFAULT_STREAM_LOAD_SEND_FPS),
            )
        elif display_state.get("dynamic_low_latency_active"):
            target_fps = min(
                target_fps,
                float(display_state.get("low_latency_send_fps", display_state.get("dynamic_low_latency_fps", target_fps)) or target_fps),
            )
        if not self._is_low_latency_mode_active(display_state):
            return 0.0
        return 1.0 / max(1.0, target_fps)

    def _refresh_low_latency_runtime_tuning(self, display_state):
        if not display_state:
            return
        mode = str(display_state.get("dynamic_throttle_mode", "idle-low-occupancy") or "idle-low-occupancy")
        if mode not in ("idle-low-bandwidth", "idle-low-framerate", "idle-low-occupancy"):
            mode = "idle-low-occupancy"
        base_fps = float(display_state.get("framerate", self.app.framerate) or self.app.framerate or TARGET_FRAMERATE)
        low_latency_fps = float(display_state.get("dynamic_low_latency_fps", min(base_fps, 15.0)) or min(base_fps, 15.0))
        try:
            crf_value = int(display_state.get("dynamic_low_latency_h264_crf", self._initial_h264_crf))
        except (TypeError, ValueError):
            crf_value = self._initial_h264_crf
        try:
            sample_percent = int(display_state.get("dynamic_low_latency_sample_percent", 75))
        except (TypeError, ValueError):
            sample_percent = 75
        sample_percent = max(10, min(100, sample_percent))
        if mode == "idle-low-framerate":
            crf_value = int(display_state.get("h264_crf", self._initial_h264_crf) or self._initial_h264_crf)
            sample_percent = 100
        elif mode == "idle-low-bandwidth":
            low_latency_fps = base_fps

        disable_paint_over = mode == "idle-low-occupancy"
        transport_level = max(0.0, min(1.0, (crf_value - 5) / 55.0))
        sample_penalty = max(0.0, min(1.0, (100 - sample_percent) / 67.0))

        low_latency_send_fps = max(1.0, min(base_fps, low_latency_fps))
        if mode == "idle-low-framerate":
            bandwidth_percent = 100
        else:
            bandwidth_percent = sample_percent
            if mode == "idle-low-occupancy":
                bandwidth_percent = min(
                    bandwidth_percent,
                    max(10, min(100, int(round((low_latency_send_fps / max(1.0, base_fps)) * 100.0)))),
                )

        high_load_send_fps = min(
            base_fps,
            max(
                12.0,
                DEFAULT_STREAM_LOAD_SEND_FPS - (transport_level * 2.5) - (sample_penalty * 3.5),
            ),
        )
        if disable_paint_over:
            high_load_send_fps = min(base_fps, max(12.0, high_load_send_fps - 1.0))

        input_priority_seconds = 0.10 + (transport_level * 0.18) + (sample_penalty * 0.08)
        if disable_paint_over:
            input_priority_seconds += 0.04

        display_state["low_latency_send_fps"] = max(1.0, min(base_fps, low_latency_send_fps))
        display_state["low_latency_bandwidth_percent"] = bandwidth_percent
        display_state["high_load_send_fps"] = max(1.0, min(base_fps, high_load_send_fps))
        display_state["input_priority_seconds"] = max(0.08, min(0.5, input_priority_seconds))
        display_state["dynamic_low_latency_disable_paint_over"] = disable_paint_over

    def _reset_transient_display_state(self, display_state):
        if not display_state:
            return
        display_state["dynamic_low_latency_active"] = False
        display_state["high_load_active"] = False
        display_state["input_priority_until"] = 0.0
        display_state["last_video_send_time"] = 0.0
        self._refresh_low_latency_runtime_tuning(display_state)

    async def _force_recover_display(self, display_id: str, restart_audio: bool = True):
        display_state = self.display_clients.get(display_id)
        if not display_state:
            data_logger.warning(f"Cannot force recover unknown display '{display_id}'.")
            return

        data_logger.info(f"Force-recovering display '{display_id}'.")
        self._reset_transient_display_state(display_state)
        display_state["video_active"] = True
        reset_sent = await self._stop_capture_for_display(display_id)
        if not reset_sent:
            await self._reset_frame_ids_and_notify(display_id)

        should_restart_audio = restart_audio and display_id == "primary"
        if should_restart_audio and self.is_pcmflux_capturing:
            await self._stop_pcmflux_pipeline()
        ...
        try:
            if not layout:
                data_logger.info(
                    f"No stored layout for '{display_id}' during force recovery. Running full reconfigure."
                )
                await self.reconfigure_displays()
            else:
            ...
            data_logger.error(
                f"Fast force recovery failed for '{display_id}', falling back to reconfigure: {exc}",
                exc_info=False,
            )
            await self.reconfigure_displays()

        if should_restart_audio and self.display_clients.get(display_id):
            try:
                await self._start_pcmflux_pipeline()
            except Exception as exc:
                data_logger.error(f"Failed to restart pcmflux during force recovery: {exc}", exc_info=False)

    async def _restart_capture_for_display_settings(self, display_id: str):
        display_state = self.display_clients.get(display_id)
        if not display_state:
            return
        if display_state.get("capture_settings_restart_in_progress"):
            return
        display_state["capture_settings_restart_in_progress"] = True
        try:
            data_logger.info(f"Restarting capture for '{display_id}' to apply dynamic inactive profile.")
            display_state["video_active"] = True
            reset_sent = await self._stop_capture_for_display(display_id)
            if not reset_sent:
                await self._reset_frame_ids_and_notify(display_id)
            ...
            data_logger.error(f"Failed to restart capture for dynamic inactive profile on '{display_id}': {exc}", exc_info=False)
            await self.reconfigure_displays()
        finally:
            if display_id in self.display_clients:
                self.display_clients[display_id]["capture_settings_restart_in_progress"] = False
    ...
    def _resolve_pcmflux_audio_device_name(self):
        requested_env = (
            os.environ.get("SELKIES_AUDIO_SOURCE")
            or os.environ.get("SELKIES_AUDIO_DEVICE_NAME")
            or os.environ.get("AUDIO_DEVICE_NAME")
            or ""
        ).strip()
        requested = (requested_env or self.audio_device_name or "").strip()
        normalized_requested = requested.lower()

        if not PULSEAUDIO_AVAILABLE:
            return requested or self.audio_device_name

        try:
            with pulsectl.Pulse("selkies-audio-source-resolver") as pulse:
                sources = {source.name: source for source in pulse.source_list()}
                default_source = ""
                try:
                    default_source = pulse.server_info().default_source_name or ""
                except Exception:
                    default_source = ""

                candidates = []
                if requested_env and requested in sources:
                    candidates.append(requested)
                elif requested and normalized_requested not in GENERIC_AUDIO_SOURCE_NAMES and requested in sources:
                    candidates.append(requested)

                if default_source:
                    candidates.append(default_source)

                candidates.extend(PREFERRED_AUDIO_SOURCE_NAMES)
                if requested:
                    candidates.append(requested)

                for source_name in candidates:
                    if source_name in sources:
                        if source_name != self.audio_device_name:
                            data_logger.info(
                                f"Resolved pcmflux audio source '{self.audio_device_name}' -> '{source_name}'"
                            )
                        self.audio_device_name = source_name
                        try:
                            pulse.default_set(sources[source_name])
                        except Exception as default_err:
                            data_logger.debug(
                                f"Could not set default PulseAudio source to '{source_name}': {default_err}"
                            )
                        return source_name

                monitor_sources = sorted(name for name in sources if name.endswith(".monitor"))
                if monitor_sources:
                    source_name = monitor_sources[0]
                    data_logger.warning(
                        f"Requested pcmflux source '{requested or self.audio_device_name}' was not found; "
                        f"falling back to monitor source '{source_name}'"
                    )
                    self.audio_device_name = source_name
                    return source_name

                data_logger.warning(
                    f"No PulseAudio monitor source found for pcmflux; available sources: {', '.join(sorted(sources))}"
                )
        except Exception as exc:
            data_logger.warning(f"Could not resolve pcmflux PulseAudio source: {exc}")
    ...
    async def _start_pcmflux_pipeline(self):
        ...
        try:
            resolved_device_name = self._resolve_pcmflux_audio_device_name()
            capture_settings = AudioCaptureSettings()
            device_name_bytes = resolved_device_name.encode('utf-8') if resolved_device_name else None
    ...
    async def _reset_frame_ids_and_notify(self, display_id: str):
        ...
        display_state['last_video_send_time'] = 0.0
        sent_timestamps = display_state.get('sent_timestamps')
        if sent_timestamps is not None:
    ...
    async def _run_frame_backpressure_logic(self, display_id: str):
        ...
        try:
            ...
            while True:
                ...
                if self._is_low_latency_mode_active(display_state) and not display_state.get('backpressure_enabled', True):
                    self._prune_video_queue_for_display(display_id, LOW_LATENCY_QUEUE_KEEP_LATEST)
    ...
    def _parse_settings_payload(self, payload_str: str) -> dict:
        ...
        parsed["encoder"] = get_str("encoder") or get_str("encoder_rtc")
        ...
        parsed["dynamic_sample_percent"] = get_int("dynamic_sample_percent")
        parsed["dynamic_low_latency_enabled"] = get_bool("dynamic_low_latency_enabled")
        parsed["dynamic_low_latency_fps"] = get_int("dynamic_low_latency_fps")
        parsed["dynamic_low_latency_h264_crf"] = get_int("dynamic_low_latency_h264_crf")
        parsed["dynamic_low_latency_sample_percent"] = get_int("dynamic_low_latency_sample_percent")
        parsed["dynamic_low_latency_disable_paint_over"] = get_bool("dynamic_low_latency_disable_paint_over")
        parsed["dynamic_throttle_mode"] = get_str("dynamic_throttle_mode")
    ...
    async def _apply_client_settings(
        ...
        def sanitize_sample_percent(value):
            if value is None:
                return old_settings.get("dynamic_sample_percent", 100)
            try:
                numeric = int(value)
            except (TypeError, ValueError):
                return old_settings.get("dynamic_sample_percent", 100)
            return max(33, min(100, numeric))

        def sanitize_low_latency_sample_percent(value):
            if value is None:
                return old_settings.get("dynamic_low_latency_sample_percent", 75)
            try:
                numeric = int(value)
            except (TypeError, ValueError):
                return old_settings.get("dynamic_low_latency_sample_percent", 75)
        ...
        def sanitize_low_latency_enabled(value, key, fallback):
            if value is None:
                return old_settings.get(key, fallback)
            return str(value).lower() in ['true', '1']

        def sanitize_low_latency_fps(value):
            if value is None:
                return old_settings.get("dynamic_low_latency_fps", 15)
            try:
                numeric = int(value)
            except (TypeError, ValueError):
                return old_settings.get("dynamic_low_latency_fps", 15)
            return max(1, min(120, numeric))

        def sanitize_low_latency_crf(value):
            if value is None:
                return old_settings.get("dynamic_low_latency_h264_crf", old_settings.get("h264_crf", self._initial_h264_crf))
            try:
                numeric = int(value)
            except (TypeError, ValueError):
                return old_settings.get("dynamic_low_latency_h264_crf", old_settings.get("h264_crf", self._initial_h264_crf))
            return max(5, min(60, numeric))

        def sanitize_throttle_mode(value):
            allowed = ("idle-low-bandwidth", "idle-low-framerate", "idle-low-occupancy")
            if value is None:
                return old_settings.get("dynamic_throttle_mode", "idle-low-occupancy")
            safe = str(value).strip()
            if safe in allowed:
                return safe
            if safe in ("bandwidth", "low-bandwidth"):
                return "idle-low-bandwidth"
            if safe in ("framerate", "low-framerate"):
                return "idle-low-framerate"
        ...
        async with self._reconfigure_lock:
            ...
            display_state["dynamic_sample_percent"] = sanitize_sample_percent(settings.get("dynamic_sample_percent"))
            display_state["dynamic_low_latency_enabled"] = sanitize_low_latency_enabled(
                settings.get("dynamic_low_latency_enabled"),
                "dynamic_low_latency_enabled",
                True,
            )
            display_state["dynamic_low_latency_fps"] = sanitize_low_latency_fps(settings.get("dynamic_low_latency_fps"))
            display_state["dynamic_low_latency_h264_crf"] = sanitize_low_latency_crf(settings.get("dynamic_low_latency_h264_crf"))
            display_state["dynamic_low_latency_sample_percent"] = sanitize_low_latency_sample_percent(settings.get("dynamic_low_latency_sample_percent"))
            display_state["dynamic_throttle_mode"] = sanitize_throttle_mode(settings.get("dynamic_throttle_mode"))
            display_state["dynamic_low_latency_disable_paint_over"] = sanitize_low_latency_enabled(
                settings.get("dynamic_low_latency_disable_paint_over"),
                "dynamic_low_latency_disable_paint_over",
                True,
            )
            self._refresh_low_latency_runtime_tuning(display_state)
            ...
            video_params_list = [
                ...
                'h264_paintover_burst_frames', 'use_paint_over_quality', 'dynamic_sample_percent'
            ]
            ...
            dynamic_low_latency_params_changed = any(
                display_state.get(key) != old_settings.get(key)
                for key in (
                    "dynamic_low_latency_fps",
                    "dynamic_low_latency_h264_crf",
                    "dynamic_low_latency_sample_percent",
                    "dynamic_throttle_mode",
                    "dynamic_low_latency_disable_paint_over",
                )
            )
            if (
                display_state.get("dynamic_low_latency_active")
                and self._is_h264_display_state(display_state)
                and dynamic_low_latency_params_changed
            ):
                video_params_changed = True
    ...
    async def ws_handler(self, websocket):
        ...
        self._set_client_awake_state(websocket, False)
        data_logger.info(f"Data WebSocket connected from {raddr}")
        # PIN-session takeover
        ...
        query_params = urllib.parse.parse_qs(urllib.parse.urlparse(websocket.request.path).query)
        query_session_id = str((query_params.get("selkies_session_id") or [""])[0] or "")
        try:
            query_session_epoch = int((query_params.get("selkies_session_epoch") or [0])[0] or 0)
        except (TypeError, ValueError):
        ...
        query_identified = (
            ...
            and query_session_id == active_session_id
            and query_session_epoch == active_session_epoch
        )
        ws_identified = (not session_enabled) or cookie_identified or query_identified
            ...
            for stale_ws in existing_clients:
                ...
                same_session_refresh = (
                    ...
                    or (
                        ...
                        and stale_meta.get("session_id", "") == active_session_id
                        and int(stale_meta.get("session_epoch", 0) or 0) == active_session_epoch
                    )
                )
        ...
        active_upload_info_conn = None
        upload_dir_valid = upload_dir_path is not None

        async def send_upload_status(status, info=None, message_text=None, force=False):
            upload_info = info or active_upload_info_conn
            if not upload_info and status != "error":
                return
            now = time.monotonic()
            if (
                not force
                and upload_info
                and status == "progress"
                and now - upload_info.get("last_status_at", 0.0) < 0.25
            ):
                return
            if upload_info is not None:
                upload_info["last_status_at"] = now
            payload = {
                "status": status,
                "fileName": upload_info.get("rel_path", "") if upload_info else "",
                "fileSize": int(upload_info.get("total_bytes", 0)) if upload_info else 0,
                "receivedBytes": int(upload_info.get("received_bytes", 0)) if upload_info else 0,
            }
            if message_text:
                payload["message"] = str(message_text)
            try:
                ...
                data_logger.debug(f"Failed to send upload status: {e_status}")
        ...
        if await asyncio.to_thread(GPUtil.getGPUs):
            ...
            async for message in websocket:
                try:
                    if isinstance(message, bytes):
                        self._bytes_received_in_interval += len(message)
                    elif isinstance(message, str):
                        self._bytes_received_in_interval += len(message.encode("utf-8", errors="ignore"))
                except Exception:
                    pass
                if isinstance(message, bytes):
                    ...
                    if msg_type == 0x01:
                        if (
                            ...
                            try:
                                ...
                                if active_upload_info_conn:
                                    active_upload_info_conn["write_calls"] = (
                                        active_upload_info_conn.get("write_calls", 0) + 1
                                    )
                                    active_upload_info_conn["write_total_ms"] = (
                                        active_upload_info_conn.get("write_total_ms", 0.0)
                                        + write_elapsed_ms
                                    )
                                    active_upload_info_conn["write_max_ms"] = max(
                                        active_upload_info_conn.get("write_max_ms", 0.0),
                                        write_elapsed_ms,
                                    )
                                    active_upload_info_conn["received_bytes"] = min(
                                        active_upload_info_conn.get("total_bytes", 0),
                                        active_upload_info_conn.get("received_bytes", 0)
                                        + len(payload),
                                    )
                                    await send_upload_status(
                                        "progress",
                                        active_upload_info_conn,
                                        force=active_upload_info_conn.get("received_bytes", 0)
                                        >= active_upload_info_conn.get("total_bytes", 0),
                                    )
                                if write_elapsed_ms >= 50.0:
                                    data_logger.warning(
                                        "Legacy upload slow write: file=%s bytes=%d duration_ms=%.2f",
                                        active_upload_info_conn.get("rel_path", "")
                                        if active_upload_info_conn
                                        else "",
                                        len(payload),
                                        write_elapsed_ms,
                                ...
                                failed_upload_info = active_upload_info_conn
                                try:
                                    active_uploads_by_path_conn[
                                        active_upload_target_path_conn
                                    ].close()
                                ...
                                await send_upload_status(
                                    "error",
                                    failed_upload_info,
                                    f"File write error: {e_write}",
                                    force=True,
                                )
                                if client_display_id:
                                    self._set_display_high_load_state(client_display_id, False, "file upload")
                    ...
                    if message.startswith("FILE_UPLOAD_START:"):
                        if 'upload' not in settings.file_transfers:
                            ...
                            await send_upload_status("error", {"rel_path": "", "total_bytes": 0, "received_bytes": 0}, "Uploads are disabled.", force=True)
                            continue
                        if not upload_dir_valid:
                            data_logger.error("Upload dir invalid, skipping upload.")
                            await send_upload_status("error", {"rel_path": "", "total_bytes": 0, "received_bytes": 0}, "Upload directory is not available.", force=True)
                            continue
                        try:
                            ...
                            if not path_components or \
                               ...
                               ".." in path_components:
                                ...
                                await send_upload_status("error", {"rel_path": rel_path_from_client, "total_bytes": file_size, "received_bytes": 0}, "Invalid upload path.", force=True)
                                continue
                            ...
                            if target_dir and target_dir != real_upload_dir_abs and not os.path.exists(target_dir):
                                if not os.path.abspath(target_dir).startswith(real_upload_dir_abs):
                                    ...
                                    await send_upload_status("error", {"rel_path": rel_path_from_client, "total_bytes": file_size, "received_bytes": 0}, "Invalid upload directory.", force=True)
                                    continue
                                try:
                                    ...
                                    await send_upload_status("error", {"rel_path": rel_path_from_client, "total_bytes": file_size, "received_bytes": 0}, f"Could not create upload directory: {e_mkdir}", force=True)
                                    continue
                            ...
                            await send_upload_status("start", active_upload_info_conn, force=True)
                            if client_display_id:
                                self._set_display_high_load_state(client_display_id, True, "file upload")
                            data_logger.info(
                            ...
                            await send_upload_status("error", {"rel_path": "", "total_bytes": 0, "received_bytes": 0}, "Invalid upload start message.", force=True)
                        except Exception as e_fup_start:
                            data_logger.error(
                                f"FILE_UPLOAD_START processing error: {e_fup_start}", exc_info=True
                            )
                            await send_upload_status("error", {"rel_path": "", "total_bytes": 0, "received_bytes": 0}, f"Upload start failed: {e_fup_start}", force=True)

                    elif message.startswith("FILE_UPLOAD_END:"):
                        finished_upload_info = active_upload_info_conn
                        finished_upload_path = active_upload_target_path_conn
                        if (
                            active_upload_target_path_conn
                            and active_upload_target_path_conn
                            in active_uploads_by_path_conn
                        ):
                            upload_file_handle = active_uploads_by_path_conn[
                                active_upload_target_path_conn
                            ]
                            flush_started_at = time.perf_counter()
                            upload_file_handle.flush()
                            flush_elapsed_ms = (time.perf_counter() - flush_started_at) * 1000.0
                            upload_file_handle.close()
                            if finished_upload_info:
                                try:
                                    finished_upload_info["received_bytes"] = os.path.getsize(active_upload_target_path_conn)
                                except OSError:
                                    finished_upload_info["received_bytes"] = finished_upload_info.get("received_bytes", 0)
                            data_logger.info(
                                "Legacy upload finished: file=%s bytes=%d elapsed_ms=%.2f "
                                "write_total_ms=%.2f write_max_ms=%.2f flush_ms=%.2f",
                                active_upload_target_path_conn,
                                int(
                                    finished_upload_info.get("received_bytes", 0)
                                    if finished_upload_info
                                    else 0
                                ),
                                (
                                    time.monotonic()
                                    - finished_upload_info.get("started_at", time.monotonic())
                                )
                                * 1000.0
                                if finished_upload_info
                                else 0.0,
                                float(
                                    finished_upload_info.get("write_total_ms", 0.0)
                                    if finished_upload_info
                                    else 0.0
                                ),
                                float(
                                    finished_upload_info.get("write_max_ms", 0.0)
                                    if finished_upload_info
                                    else 0.0
                                ),
                                flush_elapsed_ms,
                        ...
                        active_upload_info_conn = None
                        if finished_upload_info:
                            expected_bytes = int(finished_upload_info.get("total_bytes", 0))
                            received_bytes = int(finished_upload_info.get("received_bytes", 0))
                            if expected_bytes > 0 and received_bytes != expected_bytes:
                                data_logger.error(
                                    f"Upload size mismatch for {finished_upload_info.get('rel_path')}: expected {expected_bytes}, got {received_bytes}"
                                )
                                if finished_upload_path:
                                    try:
                                        os.remove(finished_upload_path)
                                    except OSError:
                                        pass
                                await send_upload_status(
                                    "error",
                                    finished_upload_info,
                                    f"Upload size mismatch: expected {expected_bytes}, got {received_bytes}",
                                    force=True,
                                )
                            else:
                                await send_upload_status("done", finished_upload_info, force=True)
                        if client_display_id:
                            self._set_display_high_load_state(client_display_id, False, "file upload")
                        ...
                        active_upload_target_path_conn = None
                        active_upload_info_conn = None
                        await send_upload_status("error", failed_upload_info, message, force=True)
                        if client_display_id:
                            self._set_display_high_load_state(client_display_id, False, "file upload")
                        ...
                        try:
                            ...
                            if display_id not in self.display_clients:
                                ...
                                self.display_clients[display_id] = {
                                    ...
                                    'dynamic_sample_percent': 100,
                                    'dynamic_low_latency_enabled': True,
                                    'dynamic_low_latency_fps': 15,
                                    'dynamic_low_latency_h264_crf': self._initial_h264_crf,
                                    'dynamic_low_latency_sample_percent': 75,
                                    'dynamic_throttle_mode': 'idle-low-occupancy',
                                    'dynamic_low_latency_disable_paint_over': True,
                                    'dynamic_low_latency_active': False,
                                    'high_load_active': False,
                                    'low_latency_send_fps': min(15, max(1, int(self.app.framerate))),
                                    'low_latency_bandwidth_percent': max(75, max(10, min(100, int(round((min(15, max(1, int(self.app.framerate))) / max(1, int(self.app.framerate))) * 100.0))))),
                                    'high_load_send_fps': min(DEFAULT_STREAM_LOAD_SEND_FPS, max(1, int(self.app.framerate))),
                                    'input_priority_seconds': self.low_latency_input_priority_seconds,
                                    'input_priority_until': 0.0,
                    ...
                    elif message.startswith("LOW_LATENCY_STATE,"):
                        if client_display_id and client_display_id in self.display_clients:
                            parts = message.split(",")
                            active = len(parts) > 1 and parts[1].strip() in ("1", "true", "True")
                            display_state = self.display_clients[client_display_id]
                            if len(parts) > 3:
                                display_state["dynamic_low_latency_fps"] = self._coerce_int_setting(
                                    parts[3].strip(),
                                    display_state.get("dynamic_low_latency_fps", 15),
                                    1,
                                    120,
                                )
                            if len(parts) > 4:
                                display_state["dynamic_low_latency_h264_crf"] = self._coerce_int_setting(
                                    parts[4].strip(),
                                    display_state.get("dynamic_low_latency_h264_crf", self._initial_h264_crf),
                                    5,
                                    60,
                                )
                            if len(parts) > 5:
                                display_state["dynamic_low_latency_sample_percent"] = self._coerce_int_setting(
                                    parts[5].strip(),
                                    display_state.get("dynamic_low_latency_sample_percent", 75),
                                    10,
                                    100,
                                )
                            if len(parts) > 6:
                                display_state["dynamic_low_latency_disable_paint_over"] = parts[6].strip() in ("1", "true", "True")
                            if len(parts) > 7:
                                mode = parts[7].strip()
                                if mode in ("idle-low-bandwidth", "idle-low-framerate", "idle-low-occupancy"):
                                    display_state["dynamic_throttle_mode"] = mode
                            self._refresh_low_latency_runtime_tuning(display_state)
                            old_low_latency_active = bool(display_state.get("dynamic_low_latency_active"))
                            display_state["dynamic_low_latency_active"] = active and display_state.get("dynamic_low_latency_enabled", True)
                            new_low_latency_active = bool(display_state.get("dynamic_low_latency_active"))
                            if new_low_latency_active and not self._should_preserve_encoded_frame_order(display_state):
                                self._prune_video_queue_for_display(client_display_id, LOW_LATENCY_QUEUE_KEEP_LATEST)
                            if not new_low_latency_active:
                                display_state["input_priority_until"] = 0.0
                            data_logger.info(
                                f"Dynamic throttling {'enabled' if new_low_latency_active else 'disabled'} for '{client_display_id}'."
                            )
                            if (
                                old_low_latency_active != new_low_latency_active
                                and self._is_h264_display_state(display_state)
                                and client_display_id in self.capture_instances
                            ):
                                await self._restart_capture_for_display_settings(client_display_id)

                    elif message.startswith("HIGH_LOAD_STATE,"):
                        if client_display_id and client_display_id in self.display_clients:
                            parts = message.split(",", 2)
                            active = len(parts) > 1 and parts[1].strip() in ("1", "true", "True")
                            reason = parts[2].strip() if len(parts) > 2 else "client hint"
                            self._set_display_high_load_state(client_display_id, active, reason)

                    elif message.startswith("CLIENT_AWAKE_STATE,"):
                        parts = message.split(",", 2)
                        active = len(parts) > 1 and parts[1].strip() in ("1", "true", "True")
                        self._set_client_awake_state(websocket, active, client_display_id)

                    elif message == "RESET_IO_MODULES":
                        if self.input_handler:
                            await self.input_handler.reset_io_modules()
                        if client_display_id and client_display_id in self.display_clients:
                            self._reset_transient_display_state(self.display_clients[client_display_id])
                        data_logger.info(
                            f"Reset input/output modules for '{client_display_id or websocket.remote_address}'."
                        )

                    elif message.startswith("FORCE_STREAM_RECOVER"):
                        target_display_id = client_display_id
                        parts = message.split(",", 2)
                        if len(parts) > 1 and parts[1].strip():
                            target_display_id = parts[1].strip()
                        if target_display_id and target_display_id in self.display_clients:
                            await self._force_recover_display(target_display_id, restart_audio=True)
                        else:
                            data_logger.warning(
                                f"Ignoring FORCE_STREAM_RECOVER for unknown display '{target_display_id}'."
                            )
                        ...
                        if client_display_id and message.split(',')[0] in ["kd", "ku", "kr", "m", "m2", "cws", "cbs", "cwd", "cbd", "cwe", "cbe", "cw", "cb"]:
                            self._register_input_priority(client_display_id)
                        if client_display_id and (
                            message.startswith("clipboard_start,")
                            or message.startswith("clipboard_binary,")
                            or message.startswith("clipboard,")
                        ):
                            self._set_display_high_load_state(client_display_id, True, "clipboard transfer")
                        ...
                        if client_display_id and (
                            message == "clipboard_finish"
                            or message.startswith("clipboard_binary,")
                            or message.startswith("clipboard,")
                        ):
                            self._set_display_high_load_state(client_display_id, False, "clipboard transfer")
    ...
    async def run_server(self):
        ...
        if self._adaptive_sleep_task is None or self._adaptive_sleep_task.done():
            self._adaptive_sleep_task = asyncio.create_task(self._adaptive_sleep_monitor())
    ...
    async def stop(self):
        ...
        if self._adaptive_sleep_task and not self._adaptive_sleep_task.done():
            self._adaptive_sleep_task.cancel()
            try:
                await self._adaptive_sleep_task
            except asyncio.CancelledError:
                pass
            self._adaptive_sleep_task = None
    ...
    async def _recover_video_queue_overflow(self, display_id, queue):
        async with self._reconfigure_lock:
            if self.video_chunk_queues.get(display_id) is queue:
                data_logger.warning(f"Video queue overflow for '{display_id}'; resetting the decoder and capture.")
                await self._restart_capture_for_display_settings(display_id)
    ...
    async def _video_chunk_sender(self, display_id: str):
        ...
        try:
            while True:
                # Queue.get() and broadcast() need not yield when frames are buffered.
                # Give input, audio and ACK processing a turn between encoded chunks.
                await asyncio.sleep(0)
                ...
                now = time.monotonic()
                if display_state:
                    preserve_frame_order = self._should_preserve_encoded_frame_order(display_state)
                    self._prune_video_queue_for_display(display_id)
                    if (
                        self._is_low_latency_mode_active(display_state)
                        and not preserve_frame_order
                        and now < display_state.get("input_priority_until", 0.0)
                        and queue.qsize() > 0
                    ):
                        queue.task_done()
                        continue
                    send_interval = self._get_effective_send_interval_seconds(display_state)
                    if (
                        send_interval > 0
                        and (now - display_state.get("last_video_send_time", 0.0)) < send_interval
                    ):
                        queue.task_done()
                        continue
                if display_id == 'primary':
                    ...
                    try:
                        ...
                        if display_state:
                            display_state["last_video_send_time"] = now
                    except Exception as e:
                    ...
                    try:
                        ...
                        client_info["last_video_send_time"] = now
                    except websockets.ConnectionClosed:
                    ...
                    if result.size > 0:
                        ...
                        if queue:
                            ...
                            def do_put():
                                ...
                                if queue_overflowed or self.video_chunk_queues.get(display_id) is not queue:
                                    return
                                try:
                                    self._prune_video_queue_for_display(display_id)
                                    queue.put_nowait(item_to_queue)
                                except asyncio.QueueFull:
                                    if encoder_for_this_capture in ("x264enc", "x264enc-striped"):
                                        # Losing a reference frame corrupts later H264 frames.
                                        # Reset once instead of silently dropping and building latency.
                                        queue_overflowed = True
                                        asyncio.create_task(self._recover_video_queue_overflow(display_id, queue))
                                        return
                                    self._prune_video_queue_for_display(display_id, LOW_LATENCY_QUEUE_KEEP_LATEST)
                                    try:
                                        queue.put_nowait(item_to_queue)
    ...
    def _get_capture_settings(self, display_id, width, height, x, y):
        ...
        if display_state.get("dynamic_low_latency_active"):
            cs.target_fps = min(
                cs.target_fps,
                float(display_state.get("low_latency_send_fps", display_state.get("dynamic_low_latency_fps", cs.target_fps)) or cs.target_fps),
            )
        cs.capture_cursor = self.capture_cursor
        cs.debug_logging = self.cli_args.debug[0]
        low_latency_active = self._is_low_latency_mode_active(display_state)
        dynamic_sample_percent = display_state.get('dynamic_sample_percent', 100)
        if low_latency_active:
            if display_state.get("dynamic_low_latency_active"):
                dynamic_sample_percent = min(
                    dynamic_sample_percent,
                    display_state.get('low_latency_bandwidth_percent', dynamic_sample_percent),
                )
            if display_state.get("high_load_active"):
                dynamic_sample_percent = min(dynamic_sample_percent, 50)
            if display_state.get("dynamic_low_latency_active"):
                dynamic_sample_percent = max(
                    dynamic_sample_percent,
                    display_state.get('dynamic_low_latency_sample_percent', dynamic_sample_percent),
                )
                if display_state.get("dynamic_throttle_mode") == "idle-low-bandwidth":
                    dynamic_sample_percent = min(
                        dynamic_sample_percent,
                        display_state.get('dynamic_low_latency_sample_percent', dynamic_sample_percent),
                    )
        try:
            dynamic_sample_percent = max(10, min(100, int(dynamic_sample_percent)))
        except (TypeError, ValueError):
            dynamic_sample_percent = 100
        ...
        if encoder == "jpeg":
            cs.output_mode = 0
            base_jpeg_quality = display_state.get('jpeg_quality', self._initial_jpeg_quality)
            base_paint_quality = display_state.get('paint_over_jpeg_quality', self._initial_paint_over_jpeg_quality)
            cs.jpeg_quality = max(10, min(100, int(round(base_jpeg_quality * dynamic_sample_percent / 100.0))))
            cs.paint_over_jpeg_quality = max(15, min(100, int(round(base_paint_quality * dynamic_sample_percent / 100.0))))
        else: # H.264 modes
            cs.output_mode = 1
            base_h264_crf = display_state.get('h264_crf', self._initial_h264_crf)
            base_paint_crf = display_state.get('h264_paintover_crf', self._initial_h264_paintover_crf)
            if low_latency_active:
                base_h264_crf = max(base_h264_crf, display_state.get('dynamic_low_latency_h264_crf', base_h264_crf))
                base_paint_crf = max(base_paint_crf, max(5, base_h264_crf - 6))
            crf_boost = int(round((100 - dynamic_sample_percent) / 3.0))
            cs.h264_crf = min(50, max(5, int(base_h264_crf) + crf_boost))
            cs.h264_paintover_crf = min(50, max(5, int(base_paint_crf) + max(1, crf_boost // 2)))
        ...
        if low_latency_active and display_state.get("dynamic_low_latency_disable_paint_over", True):
            cs.use_paint_over_quality = False
            cs.paint_over_trigger_frames = 2
            cs.damage_block_threshold = 4
            cs.damage_block_duration = 6
            if hasattr(cs, "h264_paintover_burst_frames"):
                cs.h264_paintover_burst_frames = 1
        else:
    ...
    try:
        while True:
            ...
            if elapsed_time > 0:
                ...
                upload_kbps = (server_instance._bytes_sent_in_interval * 8) / elapsed_time / 1_000
                download_kbps = (getattr(server_instance, "_bytes_received_in_interval", 0) * 8) / elapsed_time / 1_000
            else:
                current_mbps = 0.0
                upload_kbps = 0.0
                download_kbps = 0.0
            server_instance._bytes_sent_in_interval = 0
            server_instance._bytes_received_in_interval = 0
...
async def ws_entrypoint():
    ...
    if min_fr == max_fr:
        ...
        try:
            default_framerate = int(os.environ.get("SELKIES_DEFAULT_FRAMERATE", "30"))
        except (TypeError, ValueError):
            default_framerate = 30
        TARGET_FRAMERATE = max(min_fr, min(max_fr, default_framerate))
