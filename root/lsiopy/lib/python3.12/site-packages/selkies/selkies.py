...
class DataStreamingServer:
    ...
    async def _enter_adaptive_sleep(self):
        if self._adaptive_sleeping or not self.display_clients:
            return
        async with self._reconfigure_lock:
            if self._adaptive_sleeping or not self.display_clients:
                return
            data_logger.info("Adaptive sleep: no clients receiving video, stopping video/audio pipelines.")
            self._adaptive_sleep_video_active = {
    ...
    async def _adaptive_sleep_monitor(self):
        ...
        while True:
            try:
                ...
                receiving_count = self._receiving_client_count()
                if (not enabled or receiving_count > 0) and self._adaptive_sleeping:
                    await self._leave_adaptive_sleep("disabled" if not enabled else "client receiving")
                    continue
                if not enabled:
                    self._adaptive_sleep_no_awake_since = 0.0
                    continue
