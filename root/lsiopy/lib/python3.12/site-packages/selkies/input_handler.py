...
CLIPBOARD_TARGET_TIMEOUT = 3.0
CLIPBOARD_TEXT_TIMEOUT = 3.0
CLIPBOARD_DATA_TIMEOUT = 5.0
CLIPBOARD_WRITE_TIMEOUT = 5.0
CLIPBOARD_XCLIP_READY_TIMEOUT = 0.35
CLIPBOARD_REMOTE_POLL_INTERVAL = 1.5
...
class WebRTCInput:
    def __init__(
        ...
        self.gamepad_disabled = env_truthy("SELKIES_DISABLE_GAMEPAD", True)
        self.num_gamepads = 0 if self.gamepad_disabled else 4
        self.gamepad_instances = {}
        self.client_gamepad_associations = {} 

        self.clipboard_running = False
        self.clipboard_trigger_queue = asyncio.Queue(maxsize=16)
        self.last_clipboard_data_bytes = None
    ...
    async def connect(self):
        ...
        if not self.gamepad_disabled:
            await self._initialize_persistent_gamepads()
        else:
            logger_webrtc_input.info("Selkies gamepad support disabled; skipping virtual gamepad initialization.")
    ...
    async def reset_io_modules(self):
        logger_webrtc_input.info("Resetting input/output modules state.")
        ...
        try:
            while True:
                self.clipboard_trigger_queue.get_nowait()
        except asyncio.QueueEmpty:
            pass
        try:
            use_binary = self.enable_binary_clipboard in ["true", "out"]
            current_data, _ = await self.read_clipboard(use_binary=use_binary)
            if current_data is not None:
                self.last_clipboard_data_bytes = current_data.encode('utf-8') if isinstance(current_data, str) else current_data
        except Exception as e:
            logger_webrtc_input.warning(f"Failed to refresh clipboard snapshot during IO reset: {e}")
    ...
    async def send_x11_mouse(self, x, y, button_mask, scroll_magnitude, relative=False, display_id='primary'):
        ...
        if not relative and self.xdisplay:
            # Send queued input without a blocking X11 round trip per mouse event.
            self.xdisplay.flush()
    ...
    async def read_clipboard(self, use_binary=False):
        ...
        if self.is_wayland:
            try:
                ...
                stdout_types, _ = await asyncio.wait_for(proc_types.communicate(), timeout=CLIPBOARD_TARGET_TIMEOUT)
                ...
                if use_binary:
                    ...
                    if target_mime:
                        ...
                        stdout_data, _ = await asyncio.wait_for(proc_data.communicate(), timeout=CLIPBOARD_DATA_TIMEOUT)
                ...
                if any(t in available_types for t in text_mimes):
                    ...
                    stdout_text, _ = await asyncio.wait_for(proc_text.communicate(), timeout=CLIPBOARD_TEXT_TIMEOUT)
        ...
        try:
            ...
            stdout_targets, _ = await asyncio.wait_for(proc_targets.communicate(), timeout=CLIPBOARD_TARGET_TIMEOUT)
    ...
    async def write_clipboard(self, data, mime_type="text/plain"):
        ...
        if self.is_wayland:
            try:
                ...
                if process.stdin:
                    ...
                    process.stdin.close()
                stdout, stderr = await asyncio.wait_for(process.communicate(), timeout=CLIPBOARD_WRITE_TIMEOUT)
                if process.returncode == 0:
                    self.last_clipboard_data_bytes = input_bytes
                    return True
                else:
        ...
        try:
            ...
            if process.stdin:
                ...
                await process.stdin.drain()
                process.stdin.close()
            return_code = await asyncio.wait_for(process.wait(), timeout=CLIPBOARD_XCLIP_READY_TIMEOUT)
            if return_code == 0:
                self.last_clipboard_data_bytes = input_bytes
                return True
            else:
            ...
            self.last_clipboard_data_bytes = input_bytes
            logger_webrtc_input.info("xclip clipboard owner is still running; treating clipboard write as ready.")
            return True
        except Exception:
            logger_webrtc_input.warning("Error writing to clipboard with xclip", exc_info=True)
            return False
    def request_clipboard_check(self):
        if self.enable_clipboard not in ["true", "out"]:
            return
        try:
            self.clipboard_trigger_queue.put_nowait(time.monotonic())
    ...
    async def _sync_clipboard_if_changed(self, reason="trigger"):
        use_binary = self.enable_binary_clipboard in ["true", "out"]
        ...
        if curr_data_bytes == self.last_clipboard_data_bytes:
            return False

        logger_webrtc_input.info(f"Clipboard changed on {reason}. Sending content ({curr_mime})")
        await self.on_clipboard_read(curr_data, curr_mime)
        self.last_clipboard_data_bytes = curr_data_bytes
        return True

    def send_clipboard_status(self, message):
        try:
            if hasattr(self.gst_webrtc_app, "send_ws_message"):
                self.gst_webrtc_app.send_ws_message(message)
                return
            data_streaming_server = getattr(self.gst_webrtc_app, "data_streaming_server", None)
            clients = getattr(data_streaming_server, "clients", None)
            if clients:
                websockets.broadcast(clients, message)
        except Exception:
            logger_webrtc_input.warning(f"Failed to send clipboard status message: {message}", exc_info=True)

    async def start_clipboard(self):
        if self.enable_clipboard not in ["true", "out"]:
            logger_webrtc_input.info("Skipping outbound clipboard service."); return
        
        logger_webrtc_input.info(f"Clipboard remote monitor running (binary mode: {self.enable_binary_clipboard in ['true', 'out']})")
        self.clipboard_running = True
        if self.last_clipboard_data_bytes is None:
            try:
                use_binary = self.enable_binary_clipboard in ["true", "out"]
                initial_data, _ = await self.read_clipboard(use_binary=use_binary)
                if initial_data is not None:
                    self.last_clipboard_data_bytes = initial_data.encode('utf-8') if isinstance(initial_data, str) else initial_data
                    logger_webrtc_input.info("Primed remote clipboard monitor snapshot without sending initial content.")
            except Exception as e:
                logger_webrtc_input.warning(f"Failed to prime remote clipboard monitor snapshot: {e}")
        while self.clipboard_running:
            try:
                try:
                    trigger_reason = await asyncio.wait_for(
                        self.clipboard_trigger_queue.get(),
                        timeout=CLIPBOARD_REMOTE_POLL_INTERVAL
                    )
                except asyncio.TimeoutError:
                    await self._sync_clipboard_if_changed(reason="remote monitor")
                    continue
                if trigger_reason == "__stop__":
                    break
                await self._sync_clipboard_if_changed(reason="client request")
            except asyncio.CancelledError:
    ...
    def stop_clipboard(self):
        self.clipboard_running = False
        try:
            self.clipboard_trigger_queue.put_nowait("__stop__")
        except asyncio.QueueFull:
            pass
    ...
    async def on_message(self, msg, display_id='primary'):
        ...
        if msg_type == "pong":
            ...
            if self.enable_clipboard in ["true", "out"]:
                changed = await self._sync_clipboard_if_changed(reason="client request")
                if changed is False:
                    self.send_clipboard_status("clipboard_nochange")
                elif changed is None:
                    self.send_clipboard_status("clipboard_unavailable")
