...
class WebRTCInput:
    ...
    async def start_clipboard(self):
        ...
        logger_webrtc_input.info(f"Clipboard trigger service running (binary mode: {self.enable_binary_clipboard in ['true', 'out']})")
        self.clipboard_running = True
        while self.clipboard_running:
            try:
                trigger_reason = await self.clipboard_trigger_queue.get()
                if trigger_reason == "__stop__":
                    break
                await self._sync_clipboard_if_changed(reason="event trigger")
            except asyncio.CancelledError:
    ...
    async def on_message(self, msg, display_id='primary'):
        ...
        if msg_type == "pong":
            ...
            if keysym in {65507, 65508, 65511, 65512}:
                self.request_clipboard_check()
            ...
            previous_button_mask = self.button_mask
            ...
            if not (previous_button_mask & (1 << 2)) and (button_mask & (1 << 2)):
                self.request_clipboard_check()
