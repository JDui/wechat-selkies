...
search_candidates() {
    {
        ...
        xdotool search --class 'wechat' 2>/dev/null || true
        xdotool search --onlyvisible --name '微信（测试版）|WeChat Beta|WeChat' 2>/dev/null || true
        xdotool search --name '微信（测试版）|WeChat Beta|WeChat' 2>/dev/null || true
    } | awk '!seen[$0]++'
}
