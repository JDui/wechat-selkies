...
When fallback is off, the main page's capture-phase bridge intercepts the sidebar 上传文件 request before Selkies opens its hidden file input, opens a same-page floating panel containing `/uploader/`, and lets the panel handle file selection. Drag-and-drop on the main page is intercepted separately: a document-level capture handler prevents the browser from navigating to the dropped file, stops the Selkies bundle's legacy data-channel drop handler from firing, and hands `dataTransfer.files` to the iframe. Closing the panel only hides it; the iframe, Worker, and queue are retained and the sidebar 上传文件 button can reopen it. The main page otherwise only consumes upload summaries; it does not read or slice file content.

The persisted 【妙妙小工具】 switch `回退旧版上传工具` is off by default. While enabled, the bridge dynamically leaves `change`, file-drop, and sidebar 上传文件 request events untouched so Selkies's native WebSocket path receives them. When disabled, the sidebar 上传文件 button opens the standalone in-page panel. The runtime then installs the existing FileReader backpressure and WebSocket transport queue wrappers as a safety net.
