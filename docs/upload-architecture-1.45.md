...
The main Selkies page only opens the uploader and consumes `BroadcastChannel` summaries. It does not read or slice file content. The legacy WebSocket upload wrapper is disabled by default and remains available through `SELKIES_LEGACY_UPLOAD_ENABLED=true`.
